...
let db = { users: {} }; 
let activeUser = null; 
let selectedTargetUser = null; // Profilul selectat pentru autentificare 
let pinInput = ""; 
let isCreatingProfile = false; 
let setupStep = 1; 
let tempName = ""; 
let justAddedRate = null; 
let showWelcomeScreen = false; // Flag pentru ecranul de bun venit 
...
triggerHaptic(); 
selectedTargetUser = selectedTargetUser = name ? null : name; 
pinInput = ""; 
render(); 
}
...
if (selectedTargetUser && db.users[selectedTargetUser]) { 
if (db.users[selectedTargetUser].pin = pinInput) { 
matchedUser = selectedTargetUser; 
} 
} else { 
matchedUser = Object.keys(db.users).find(u => db.users[u].pin = pinInput); 
}

if (matchedUser) {
  pinInput = "";
  selectedTargetUser = null;
  triggerWelcome(matchedUser);
} else {
  triggerHaptic();
  alert("PIN incorect pentru profilul selectat!");
  pinInput = "";
  render();
}
} 
...
function triggerWelcome(userName) { 
activeUser = userName; 
...
}, 1400); // Durata ecranului de bun venit 
}
...
return <div class="theme-toggle"> <button class="theme-btn ${currentTheme === 'light' ? 'active' : ''}" onclick="setTheme('light')">☀️ Light</button> <button class="theme-btn ${currentTheme === 'dark' ? 'active' : ''}" onclick="setTheme('dark')">🌙 Dark</button> </div>; 
}
...
return <div class="pin-container"> <div class="pin-dots"> ${[0,1,2,3].map(i =><div class="pin-dot ${i < pinInput.length ? 'active' : ''}"></div>).join('')} </div> <div class="keypad"> ${[1,2,3,4,5,6,7,8,9].map(n => <button class="keypad-btn ios-btn" onclick="handleKeypress('${n}')">${n}</button>).join('')} <button class="keypad-btn action-btn ios-btn" style="color:var(--danger)" onclick="clearPin()">Șterge</button> <button class="keypad-btn ios-btn" onclick="handleKeypress('0')">0</button> <button class="keypad-btn action-btn" style="opacity:0" disabled></button> </div> </div> ; 
}
...
return <div class="card"> <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;"> <span style="font-size: 12px; font-weight: 700; color: var(--ink-soft); text-transform: uppercase; letter-spacing: 0.5px;">🏆 Top 5 Săptămânal</span> <span style="font-size: 11px; color: var(--ink-faint);">Live Sync</span> </div> <div> ${top5.map((item, index) => { const isRank1 = index === 0; const isMe = item.name === activeUser; return 
...
// Ecran Welcome animat 
if (showWelcomeScreen && activeUser) { 
root.innerHTML = <div class="welcome-overlay"> <div class="welcome-avatar">${activeUser.charAt(0).toUpperCase()}</div> <div style="font-size: 26px; font-weight: 800; color: var(--ink); margin-bottom: 6px;">Salut, ${activeUser}!</div> <div style="font-size: 15px; color: var(--ink-soft);">Sesiune activată cu succes</div> </div>; 
return; 
}
...
root.innerHTML = <div style="display:flex; justify-content:flex-end; margin-bottom:10px;">${renderThemeToggle()}</div> <div style="padding-top: 20px; text-align: center;"> <div style="font-size: 32px; font-weight: 800; color: var(--ink); margin-bottom: 6px; letter-spacing: -0.5px;">REVIZIO</div> <p style="color: var(--ink-soft); font-size: 15px; margin-bottom: 32px;">Configurare profil nou</p> <div class="card"> <input type="text" id="nameInput" class="input-field" placeholder="Numele tău (ex: Marius)" /> <button class="btn-main ios-btn" onclick=" const val = document.getElementById('nameInput').value.trim(); if(val) { tempName = val; setupStep = 2; render(); } else { alert('Introdu un nume valid'); } ">Pasul Următor</button> </div> </div>; 
return; 
}
...
if (isCreatingProfile && setupStep = 2) { 
root.innerHTML = <div style="display:flex; justify-content:flex-end; margin-bottom:10px;">${renderThemeToggle()}</div> <div style="padding-top: 10px; text-align: center; display:flex; flex-direction:column; flex:1;"> <div style="font-size: 24px; font-weight: 700; color: var(--ink); margin-bottom: 6px; letter-spacing: -0.3px;">Setează un PIN (4 cifre)</div> <p style="color: var(--ink-soft); font-size: 15px;">Profil: <b style="color:var(--ink);">${tempName}</b></p> ${renderKeypad()} </div>; 
return; 
}
...
<p style="color: var(--ink-soft); font-size: 15px; margin-bottom: 16px;"> 
${userNames.length > 1 ? 'Alege profilul tău sau introdu PIN-ul' : 'Introdu codul PIN'} 
</p>

    ${userNames.length > 0 ? `
      <div class="profile-picker-grid">
        ${userNames.map(name => `
          <button class="profile-chip ios-btn ${selectedTargetUser === name ? 'selected' : ''}" onclick="selectTargetUser('${name}')">
            👤 ${name}
          </button>
        `).join('')}
      </div>
